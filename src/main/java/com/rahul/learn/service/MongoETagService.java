...
public class MongoETagService implements ETagService {
	...
	public String generate(String url, Object entity) {
		...
		BasicDBObject dbo = new BasicDBObject().append("url", url).append(
				"tag", tag);
	...
	public String update(String url, Object entity) {
		String tag = createTag(entity);
		BasicDBObject dbo = new BasicDBObject().append("url", url);
		mongoOps.getCollection("etags").update(
				dbo,
				new BasicDBObject().append("$set",
						new BasicDBObject().append("tag", tag)));
		return tag;
	}

	public String get(String url) throws Exception {
		BasicDBObject dbo = new BasicDBObject().append("url", url);
		List<DBObject> results = mongoOps.getCollection("etags").find(dbo)
				.toArray();
		if (results.size() == 0) {
			throw new Exception("No tag found for url: " + url);
		}
